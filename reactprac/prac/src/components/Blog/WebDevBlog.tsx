...
function WebDevBlog({ isDarkMode, toggleDarkMode }: { isDarkMode: boolean; toggleDarkMode: () => void }) {
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-gray-900 text-teal-100' : 'bg-white text-gray-900'}`}>
      ...
      <main className="flex-1 max-w-4xl mx-auto p-4">
        <h1 className={`text-4xl ${isDarkMode ? 'text-teal-100' : 'text-gray-900'}`}>Web Dev blows and I love it</h1>
        <br />
        <p className={`mb-8 ${isDarkMode ? 'text-teal-100' : 'text-gray-900'}`}>
        As a self-taught programmer, figuring out which 'lane' to go
